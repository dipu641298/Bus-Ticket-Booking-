...
create or replace package body searchAvail as
	...
	cursor cur
	is
	select bid, bname, price, avail from bus 
	where avail > 0 and rid in( select rid from route where fromto = r and pickup_point = 'mtjl');
	...
	procedure isAvail2(r in out route.fromto%type, d in out schedule.dates%type)
		...
		in(select rid from route where fromto = r and pickup_point = 'mtjl'));
